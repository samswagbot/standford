...
const backLinkStyles =
  "mb-6 inline-flex items-center gap-2 rounded-md border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-300 transition hover:border-slate-600 hover:bg-slate-800 hover:text-white";

export default function ApodDetailPage() {
  const { date } = useParams<{ date: string }>();

  const selectedApod = date ? getSelectedApod() : null;
  const apod = selectedApod?.date === date ? selectedApod : null;
  ...
  const backLink = (
    <Link href="/" className={backLinkStyles}>
      ← Back to gallery
    </Link>
  );
